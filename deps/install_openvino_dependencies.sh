...
if [ -n "$selftest" ] ; then
    ...
    for opt in "-h" "-p" "-e -p" "-n" "-n -e" "-y" "-y -e"; do
        ...
        docker run -it --rm \
            ...
            -e DEBIAN_FRONTEND=noninteractive \
            ubuntu:26.04 \
            bash "/scripts/${0##*/}" "$opt" --keepcache 2>/dev/null || true
        echo "||"
        echo "|| Completed: ubuntu:26.04 / '$opt'"
        echo "||"
    done

    echo "Self test finished, to remove temporary docker volumes run:"
    echo "'docker volume rm yum-cache apt-cache'"
    exit 0
fi
...
if [ "$os" == "raspbian9" ] || [ "$os" == "debian9" ]; then
    ...
    elif [ "$os" == "ubuntu25.04" ] || [ "$os" == "ubuntu26.04" ]; then
...
if [ "$os" == "debian9" ] || [ "$os" == "raspbian9" ] || [ "$os" == "ubuntu18.04" ] ||
   ...
   [ "$os" == "debian12" ] || [ "$os" == "raspbian12" ] || [ "$os" == "debian13" ] || [ "$os" == "ubuntu22.10" ] || [ "$os" == "ubuntu23.04" ] || [ "$os" == "ubuntu23.10" ] || [ "$os" == "ubuntu24.04" ] || [ "$os" == "ubuntu26.04" ] ; then
    ...
    [ -n "$extra" ] && [ ${#extra_repos[@]} -ne 0 ] && $PKGCMD localinstall ${iopt:+$iopt} --nogpgcheck "${extra_repos[@]}"
